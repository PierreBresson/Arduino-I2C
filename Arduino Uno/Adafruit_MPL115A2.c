...
void Adafruit_MPL115A2_readCoefficients() {
  ...
  TwoWire_beginTransmission(MPL115A2_ADDRESS);
  i2cwrite((uint8_t)MPL115A2_REGISTER_A0_COEFF_MSB);
  TwoWire_endTransmission(1);

  TwoWire_requestFrom(MPL115A2_ADDRESS, 8);
...
void Adafruit_MPL115A2_getPT(float *P, float *T) {
  ...
  TwoWire_requestFrom(MPL115A2_ADDRESS, 4);
